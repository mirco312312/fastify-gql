...
module.exports = class SubscriptionConnection {
  ...
  handleConnectionInit(data) {
    if (this.onConnect) {
      try {
        const connectionParams = this.onConnect(data.payload, this.socket) || {}
        this.connectionContext = { headers: connectionParams }
      } catch (e) {
        this.sendMessage(GQL_ERROR, id, e.message)
        this.handleConnectionClose()
        return;
    ...
    if (this.keepAlive) {
      const keepAliveTimer = setInterval(() => {
        if (this.socket.readyState === WebSocket.OPEN) {
          this.sendMessage(GQL_KEEP_ALIVE)
        } else {
  ...
  async handleGQLStart (data) {
    ...
    if (result.errors) {
      this.sendMessage(GQL_ERROR, data.id, result.errors)
      return
    }
